  ...
  el.innerHTML = `
    ...
    <div class="grid cols-4">
      <div class="card kpi"><div class="label">kredyt spłacona</div>
        ...
        <div class="sub">start ${fmt.pln(d.lodz_start)} · po spłacie surplus +${fmt.pln(d.lodz_freed_monthly)}/mies.</div></div>
    ...
    <div class="card mt">
      ...
      <div style="overflow-x:auto"><table>
        ...
        <tbody>${rows.map((r) => `<tr class="${r.below_buffer ? "cf-warn" : ""}">
          ...
          <td style="text-align:right" class="${r.lodz_balance === 0 ? "pos" : "muted"}">${r.lodz_balance === 0 ? "spłacona 🎉" : fmt.pln(r.lodz_balance)}</td>
        </tr>`).join("")}</tbody>
      </table></div>
    </div>`;
  ...
  trackChart(new Chart(document.getElementById("cfChart"), {
    ...
    data: {
      ...
      datasets: [
        ...
        { label: "Saldo kredytu", data: rows.map((r) => r.lodz_balance),
