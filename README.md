...
- **Market-barometer collectors (n8n)** — the Career tab's barometer tracks demand for **your roles** (configurable roles + geography, edited in the tab). Two importable monthly workflows feed it from popular job boards — [`barometer-jsearch-collector.json`](integrations/n8n/) (Google for Jobs: LinkedIn/Indeed/Glassdoor aggregate) and [`barometer-apify-collector.json`](integrations/n8n/) (Apify LinkedIn/Indeed). Shown as an **index (base 100) + trend**, not a raw count — see the collectors' README for the methodology.
